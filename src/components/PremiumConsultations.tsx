...
import { SignatureNumerologyPage } from '../modules/signature/SignatureNumerologyPage';
...
interface PremiumConsultationsProps {
  activeToolFromRoute?: 'VEHICLE' | 'HOUSE' | 'BUSINESS' | 'SIGNATURE' | 'CHILD' | 'LUCKY_DATES';
}
...
export default function PremiumConsultations({ activeToolFromRoute }: PremiumConsultationsProps = {}) {
  ...
  React.useEffect(() => {
    if (activeToolFromRoute) {
      setActiveModule(activeToolFromRoute);
    }
  }, [activeToolFromRoute]);
  ...
  const moduleRouteMap: Record<string, string> = {
    'VEHICLE': 'vehicle-numerology',
    'HOUSE': 'house-numerology',
    'BUSINESS': 'business-numerology',
    'SIGNATURE': 'signature-numerology',
    'CHILD': 'child-numerology',
    'LUCKY_DATES': 'lucky-date-finder'
  };
    ...
    <div id="premium-consultations-panel" className="grid grid-cols-1 lg:grid-cols-12 gap-8 text-left max-w-7xl mx-auto animate-in fade-in duration-500">
      ...
      <div className="lg:col-span-8 bg-[#FDFCF7] border border-[#F2E8DC] rounded-[40px] p-8 md:p-10 shadow-sm min-h-[550px]">
        ...
        {activeModule === 'SIGNATURE' && (
          <SignatureNumerologyPage language={lang} />
        )}
